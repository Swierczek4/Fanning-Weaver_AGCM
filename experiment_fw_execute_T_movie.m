...
Q_old = zeros(Ny,Nx);
Q = Q_old;
T_Time_Series = zeros(num_steps,1);
PRECIP_Time_Series = zeros(num_steps,1);
[PRECIP,Q] = fcn_precip(T,rho_air,rho_sea,Hq,Q,dt);
%% end ICs
...
colormap(cm)
contourf(LON_GRID,LAT_GRID,T_old-273.15,'LineStyle','none','LevelList',z7);
...
xtickformat('degrees')
ytickformat('degrees')
title('surface air temperature [deg C] on day 1',...
...
vidObj = VideoWriter('movie_TEMP.avi');
vidObj.Quality = 100;
vidObj.FrameRate = 32;
open(vidObj);
...
%% time stepping
for ii=1:num_steps
    T_Time_Series(ii) = mean(mean(T));
    PRECIP_TEMP = fcn_precip(T,rho_air,rho_sea,Hq,Q,dt);
    PRECIP_Time_Series(ii) = mean(mean(PRECIP_TEMP));
    fprintf('mean precip: %g  mean q: %g  at step %g\n',PRECIP_Time_Series(ii),mean(mean(Q)),ii)
    if (mod(ii,7)==1)||(mod(ii,13)==1)
        [T_new,Q_new,PRECIP] = ode_fw_fe(T,SST,Q,Co,...
            rho_air,rho_sea,epsO,Ha,Hq,Lnu,So,...
            sigma,Crhoa,DY,DX,Coal,epsP,epsA,...
            nu,kappa,S,U,sea,qsatSST,dt,PRECIP);
    else
        [T_new,Q_new,PRECIP] = ode_fw_lf(T,T_old,SST,Q,Q_old,Co,...
            rho_air,rho_sea,epsO,Ha,Hq,Lnu,So,...
            sigma,Crhoa,DY,DX,Coal,epsP,epsA,...
            nu,kappa,S,U,sea,qsatSST,dt,PRECIP);
    end
    ...
    if (mod(ii,16)==1)&&(ii<num_steps/5)
        contourf(LON_GRID,LAT_GRID,T-273.15,'LineStyle','none','LevelList',z7);
        colorbar('eastoutside');
        hold on
        contour(LON_GRID,LAT_GRID,0+land,'Color','k')
        caxis(coloraxis7)
        ...
        xtickformat('degrees')
        ytickformat('degrees')
        title(['surface air temperature [deg C] on day ',num2str(1+floor(ii*dt/86400))],...
            'FontWeight','Normal','FontSize',16)
        xlabel('Longitude')
        ylabel('Latitude')
        acc_movie
    ...
    if  sum(sum(isnan(T)))>1
        disp('scheme has blown up!')
        break
    elseif  sum(sum(isnan(Q)))>1
        disp('scheme has blown up!')
        break
        %     elseif min(min(Q))<0
