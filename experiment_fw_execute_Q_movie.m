...
colormap(cm)
contourf(LON_GRID,LAT_GRID,output_Q,'LineStyle','none','LevelList',z5);
...
xtickformat('degrees')
ytickformat('degrees')
title('specific humidity [kg/kg] on day 1',...
...
vidObj = VideoWriter('movie_Q.avi');
vidObj.Quality = 100;
vidObj.FrameRate = 32;
open(vidObj);
...
for ii=1:num_steps
    ...
    if (mod(ii,16)==1)&&(ii<num_steps/5)
        ...
        xtickformat('degrees')
        ytickformat('degrees')
        title(['specific humidity [kg/kg] on day ',num2str(1+floor(ii*dt/86400))],...
            'FontWeight','Normal','FontSize',16)
        xlabel('Longitude')
        ylabel('Latitude')
        acc_movie
